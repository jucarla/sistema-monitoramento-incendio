...
const char* ssid = "VIVOFIBRA-4638";  // Rede WiFi
const char* password = "BBA16352FA";  //Senha da Rede WiFi
...
void setup() {
  ...
  mySUART.begin(115200);//Comunicacao msp
...
void handle_OnConnect() {
  ...
  Serial.println("[Leitura do sensor de nível (cheio ou vazio)]"); //Imprime no monitor serial o valor da umidade lida
...
String EnvioHTML(float temperaturaStat, float umidadeStat) { //Exibindo a página da web em HTML
  ...
  ptr += "[Leitura do sensor de nível (cheio ou vazio)]";
  ptr += " %</p>";
